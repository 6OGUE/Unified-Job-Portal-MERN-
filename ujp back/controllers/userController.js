...
const performSmartVerification = async (fileBuffer, nameToVerify) => {
    ...
    if (!normalizedNameToVerify) {
        throw new Error('Name to verify is empty.');
    }
...
async function registerEmployer(req, res) {
    ...
    const cleanupFile = (file) => {
        if (file?.filename && fs.existsSync(path.join(baseUploadPath, file.filename))) {
            try { fs.unlinkSync(path.join(baseUploadPath, file.filename)); } catch (e) {}
        }
    };
    ...
    try {
        ...
        const user = new User({
            ...
            certificateHash: fileHash, certificateFilePath: certificateFilePath,
        });
...
async function registerJobSeeker(req, res) {
    ...
    const cleanupFiles = () => allUploadedFiles.forEach(file => {
        if (file?.filename && fs.existsSync(path.join(baseUploadPath, file.filename))) {
            try { fs.unlinkSync(path.join(baseUploadPath, file.filename)); } catch (e) {}
        }
    });
    ...
    try {
        ...
        if (!await performSmartVerification(cvFileBuffer, name)) {
            cleanupFiles();
            return res.status(400).json({ message: 'Verification failed: Your name was not found in the uploaded CV.' });
        }
        ...
        const user = new User({
            name, email, password, role, dateOfBirth: dateOfBirth ? new Date(dateOfBirth) : undefined, gender, about,
            education, cvFilePath: saveFileAndGetUrl(cvFile, req), certificates: certificateRecords, linkedinProfile,
            githubProfile, skills: skills ? skills.split(',').map(s => s.trim()) : [],
...
export const uploadCV = async (req, res) => {
    ...
    try {
        ...
        if (!await performSmartVerification(fileBuffer, user.name)) {
            cleanupFile();
            return res.status(400).json({ message: `Verification failed: Your name was not found in the new CV.` });
        }
...
export const addCertificates = async (req, res) => {
    ...
    const cleanupFiles = () => allFiles.forEach(file => {
        if (file?.filename && fs.existsSync(path.join(baseUploadPath, file.filename))) {
            try { fs.unlinkSync(path.join(baseUploadPath, file.filename)); } catch (e) {}
        }
    });
    ...
    try {
        ...
        for (const file of allFiles) {
            ...
            if (!await performSmartVerification(fileBuffer, user.name)) {
                cleanupFiles();
                return res.status(400).json({ message: `Verification failed: Your name was not found in the certificate '${file.originalname}'.` });
            }
            ...
            newVerifiedCertificates.push({
                title, filePath: saveFileAndGetUrl(file, req), mimetype: file.mimetype,
            });
        }
